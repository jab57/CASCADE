...
CASCADE follows a layered design (\autoref{fig:architecture}). The MCP server exposes 26 tools organized into eight categories: workflow orchestration, perturbation simulation, gene similarity, network vulnerability, experimental data, druggability assessment, TF regulon validation, and CRISPR essentiality. Alongside tools, the server provides five **MCP Resources**---browsable read-only endpoints (`cascade://cell-types`, `cascade://lincs/summary`, `cascade://model/status`, and URI templates for per-cell-type network summaries and per-gene metadata)---that allow clients to discover available data and model status without invoking a full analysis.
...
For a transcription factor, a request to `comprehensive_perturbation_analysis` with `gene="MYC"`, `cell_type="epithelial_cell"`, and `perturbation_type="knockdown"` identifies MYC as a master regulator with 427 direct network targets and 4,358 downstream affected genes, validates its TF classification against DoRothEA curated regulons (confidence A, 384 high-confidence targets including TERT and HDAC2), reports super-enhancer association in 32 cell types (BRD4-sensitive), returns 25 STRING protein interaction partners, and reports DepMap CRISPR essentiality: MYC is essential in 95% of cancer cell lines (Chronos mean −1.98), flagging it as a common essential gene with potential broad toxicity risk. The evidence synthesis identifies 24 genes supported by multiple independent sources, with STAT3 appearing across the network, STRING, and DoRothEA layers simultaneously.
