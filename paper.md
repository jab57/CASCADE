...
For a tumor-state network, the same workflow accepts `network_source="tcga"` and `tcga_network="brca"` in place of a cell type. Querying `gene="ESR1"` in the TCGA BRCA network classifies ESR1 as a master regulator with 170 direct network targets and 3,080 downstream affected genes. All top-scoring predicted effects are downregulations, with the highest-confidence direct targets including GATA3 (edge weight 0.9999), AGR3 (0.9999), CCDC170 (1.000), XBP1 (0.9998), MYB (0.9998), GREB1 (0.9993), FOXA1, and PGR---the canonical ER-positive luminal identity program in breast cancer. This result is consistent with the established role of ESR1 as the master regulator of luminal differentiation in ER+ BRCA: its loss collapses the estrogen-receptor-driven transcriptional state, suppressing the GATA3→FOXA1 pioneer circuit and downstream effectors (GREB1, CA12, CCDC170, RERG, MYB) that define the luminal subtype. Because TCGA ARACNe networks store gene symbols rather than Ensembl IDs, perturbation analysis on TCGA networks uses pure network propagation without embedding-based similarity weighting; the network topology derived from primary tumor expression data provides the tumor-context specificity.
