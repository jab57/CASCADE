...
CASCADE follows a layered design (\autoref{fig:architecture}). The MCP server exposes 25 tools organized into seven categories: workflow orchestration, perturbation simulation, gene similarity, network vulnerability, experimental data, druggability assessment, and TF regulon validation. Alongside tools, the server provides five **MCP Resources**---browsable read-only endpoints (`cascade://cell-types`, `cascade://lincs/summary`, `cascade://model/status`, and URI templates for per-cell-type network summaries and per-gene metadata)---that allow clients to discover available data and model status without invoking a full analysis.

![CASCADE architecture. An MCP client sends a request to the CASCADE server, which routes it through a LangGraph workflow. The workflow classifies the gene, selects analyses based on gene role and depth, and executes independent batches in parallel. Analysis tools operate on pre-computed regulatory networks and gene embeddings, while external modules query STRING, LINCS, dbSUPER, and DoRothEA.\label{fig:architecture}](figure_architecture.png)
...
Once running, any MCP-compatible client can call CASCADE tools. For example, a request to `comprehensive_perturbation_analysis` with `gene="MYC"`, `cell_type="epithelial_cell"`, and `perturbation_type="knockdown"` identifies MYC as a master regulator with 427 direct network targets and 4,358 downstream affected genes, validates its TF classification against DoRothEA curated regulons (confidence A, 384 high-confidence targets including TERT and HDAC2), reports super-enhancer association in 32 cell types (BRD4-sensitive), and returns 25 STRING protein interaction partners. The evidence synthesis identifies 24 genes supported by multiple independent sources, with STAT3 appearing across the network, STRING, and DoRothEA layers simultaneously.
...
CASCADE uses pre-trained gene embeddings from the GREmLN model developed by the Chan Zuckerberg Initiative AI team. We acknowledge the STRING Consortium, the LINCS Program, dbSUPER, and the DoRothEA/decoupler projects for providing the external datasets and tools integrated into CASCADE. CASCADE uses PyTorch [@pytorch] for model inference and LangGraph for workflow orchestration.
